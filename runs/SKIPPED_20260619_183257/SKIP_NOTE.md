# b_decide SKIPPED — 2026-06-19 (decide_power fire)

Tentative run_id: 20260619_183257 (UTC)
Resolved run_id:  20260529_123901  (latest available today_watchlist.json — 21 DAYS OLD)

## Why skipped (fail-closed)

1. **Stale watchlist.** Today is 2026-06-19 PT. The most recent `today_watchlist.json`
   is from `20260529_123901` (2026-05-29) — 21 calendar days old. Stage 2
   ...
   list either. Dispatching adversarial debate against three-week-old candidates
   would anchor decisions on dead setups; entry/stop levels are no longer
   meaningful and overnight news context is irrelevant.

2. **Malformed open_positions.md.** `compute_risk_budget()` still raises on the
   BSX row in `wiki/meta/open_positions.md`:
...
Sixth consecutive b_decide skip with the same reasoning:
...
- 3f0d9e7b  b_decide SKIPPED 2026-06-18 (stale watchlist 20d, fail-closed)
- e62c1dfe  b_decide SKIPPED 2026-06-19 decide_open (stale watchlist 21d, fail-closed)
- (this one) b_decide SKIPPED 2026-06-19 decide_power (stale watchlist 21d, fail-closed)
...
- Stage 3 init's wiki-injection side effect on `runs/20260529_123901/facts/`
  was reverted via `git checkout -- runs/20260529_123901/facts/` before commit.
